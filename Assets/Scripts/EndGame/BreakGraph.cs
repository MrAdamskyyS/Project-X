...
public class BreakGraph : MonoBehaviour
{
    ...
    private void Awake()
    {
        graphContainer = transform.Find("graphContainer1").GetComponent<RectTransform>();
        ShowGraph(CollectLap.Instance.brakes);
    }
    ...
    private void ShowGraph(List<float> speeds)
    {
        ...
        float yMaximum = SettingsDataCollector.Instance.brakeForceValue;
        float xSize = 3f;
